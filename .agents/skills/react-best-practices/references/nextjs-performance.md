...
Use a supported post-response facility only for nonessential work whose failure is tolerable. Await required writes. Use a durable job system for work that must survive process exit. A background callback does not guarantee completion.
