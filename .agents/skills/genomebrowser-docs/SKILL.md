...
1. Read the existing pages and relevant repository instructions. Find the canonical explanation and affected examples, API tables, tutorials, troubleshooting entries, and architectural claims. Verify behavioral claims against public entry points, implementation, and tests; distinguish maintainer intent from implemented behavior.
2. For a restructuring or package pass, inventory pages, inbound links, packaged contents, and public exports. Map topics to keep, move, merge, rewrite, or retire, and establish the page map before writing. Work package by package within the authorized scope; an accepted map does not need repeated approval.
