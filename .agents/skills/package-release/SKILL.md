...
2. Present the affected packages, consumer changes, and maintainer-specified or recommended versions. Resolve consequential uncertainty in version or channel selection before applying it. Do not infer a beta requirement from existing configuration.
...
6. When separately requested, publish the GitHub releases after confirming npm publication. Verify their tags, target commits, prerelease status, and resulting links. Report completed releases and any remaining work, distinguishing drafts from published releases.
