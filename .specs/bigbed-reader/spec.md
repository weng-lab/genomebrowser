...
- **R2:** Factory options must contain a public HTTP(S) `url` and a required Zod `schema`. The server must allow cross-origin range requests; exposing `Content-Range` to browser JavaScript is recommended but not required.
...
- **R17:** Range access must reject a `200` response, an invalid or inconsistent accessible `Content-Range`, an accessible response for different bytes, a body shorter or longer than the requested span, and transport-level `Content-Encoding`. A missing or CORS-inaccessible `Content-Range` may be accepted only when the response is `206` and the body length exactly matches the requested span.
...
Range requests use inclusive HTTP byte bounds derived from an internal offset and length. The reader requires `206`, absence of transport `Content-Encoding`, and an exact body length. When `Content-Range` is accessible, it must match the requested offsets; when browser CORS policy hides it, status and body length provide a weaker compatibility check. A server returning the complete resource with `200 OK` is unsupported, even when the requested bytes could be sliced from that body, because doing so can unexpectedly download a very large file.
...
- Verify acceptance when `Content-Range` is absent or unavailable to browser JavaScript but the `206` body length is exact, plus rejection of incorrectly sized bodies and transport-level `Content-Encoding`.
...
- Some CORS-enabled servers do not expose `Content-Range`. The reader accepts exact-size `206` responses from those servers for compatibility, but cannot prove the returned starting offset. Intermediaries that expose transport compression remain unsupported.
...
### A004 - Cache immutable file metadata per file object

- **Supersedes:** R14; the statements in **Desired Outcome** and **Private BBI boundary** that describe BBI infrastructure and every read as stateless; the verification requirement that repeated reads repeat metadata work; the **Out of Scope** exclusion of metadata, chromosome-tree, and cross-read caching; and the accepted repeated metadata work described in **Risks and Edge Cases**.
- **Replacement:** `createBigBedFile()` remains synchronous and performs no network request. The returned file object owns a private, lifetime-scoped cache of successfully loaded immutable metadata. The first `read()` for that object loads and caches the common BBI header. Successful chromosome lookups are cached by queried chromosome name, including an unknown-chromosome result, and subsequent reads on the same object reuse the cached header and chromosome result. A newly created file object starts with an empty cache and provides the explicit way to refresh metadata for a URL whose contents may have changed. Regional-index nodes, data blocks, decoded records, and complete query results remain uncached, so each read still traverses the regional index and loads data for its own region.

  Cache population must not weaken per-read cancellation isolation. An aborted or failed metadata load must not populate the cache and a later read must be able to retry. Concurrent cache misses must not make one read's `AbortSignal` own or cancel another read's metadata work; the initial implementation may perform duplicate loads for concurrent misses rather than sharing an in-flight signal-bound promise. Once metadata has been cached successfully, using it requires no fetch and must still respect cancellation checks before proceeding to read-owned work. Cache state and BBI types remain private and do not change the public `GenomicFile` contract.

  Verification must prove that factory creation remains request-free; the first successful read loads metadata; repeated same-chromosome reads reuse the header and chromosome lookup while repeating regional-index and data-block access; different chromosome names share the header but perform and then cache their own lookup; unknown chromosomes are cached; separate file objects do not share metadata; aborted and failed loads are retryable; and concurrent reads retain independent cancellation behavior. Benchmarking against the installed legacy reader should be used to quantify the effect, but no specific timing threshold is a correctness requirement.
- **Reason:** Real-file measurements showed that stateless reads repeat 10–11 small sequential requests and make warm reads materially slower than the legacy reader. Header and chromosome metadata are immutable for the lifetime of a file object and can be reused without caching query-dependent data or changing synchronous creation.

### A005 - Cache the primary R-tree header per file object

- **Supersedes:** A004 where it limits cached metadata to the common BBI header and chromosome lookup results and states that every read traverses all regional-index metadata from the beginning.
- **Replacement:** The file object's private immutable metadata cache also retains the successfully loaded and parsed primary R-tree header. The first read that reaches regional-index traversal loads it from the common header's unzoomed index offset; subsequent reads on the same file object reuse it and begin traversal at its root. Failed or aborted loads must not populate the cache, concurrent misses must retain independent cancellation behavior, and a new file object must start with an empty R-tree header cache. Regional-index nodes, block references, data blocks, decoded records, and complete results remain uncached.

  The regional-index module must expose only the narrow private type and operation needed to pass a parsed tree header into traversal; this remains package-internal and must not change the public reader API. Verification must distinguish the R-tree header range from root/node and data ranges, prove only the header is reused, and cover retry and cancellation behavior proportionately through existing metadata-cache seams.
- **Reason:** Warm real-file reads still perform the same immutable 48-byte primary-index header request before every regional traversal. Caching it is a small extension of the established per-file metadata lifetime without introducing the memory growth or invalidation concerns of index-node caching.

### A006 - Read each primary R-tree node in one request when resource size is known

- **Supersedes:** R23 and related verification language where variable-sized regional-index nodes always require separate exact header and body requests, plus the exclusion of measured read-ahead where it conflicts with this amendment.
- **Replacement:** When a valid, CORS-visible `Content-Range` provides the complete resource size, the file object may retain that immutable size as private metadata. Given the cached primary R-tree `blockSize`, regional traversal must then request each node's complete maximum encoded span in one exact range, capped at the known resource boundary, and parse only the node type, declared item count, and bytes belonging to those items. Read-ahead bytes must not be interpreted as part of the node. The maximum span must safely cover either internal or leaf items and retain existing item-count, offset, allocation, and binary-bounds validation.

  If the complete resource size is unavailable because `Content-Range` is absent, inaccessible, or uses `*`, traversal must preserve the existing exact two-request node path. Resource-size discovery must not require an extra request, weaken strict `206` validation, accept a short response, or make a hidden header mandatory. Failed or aborted requests must not populate metadata, concurrent reads retain independent signals, and a new file object starts without a known size. R-tree nodes remain uncached; each read still requests every traversed node. Focused tests must prove one request per node with known size, fallback behavior without size, exact boundary capping, and unchanged rejection and cancellation semantics.
- **Reason:** Real-browser measurements show that sequential range-request latency dominates decoding. One exact read-ahead request per node reduces round trips without retaining nodes or approaching the legacy reader's broad buffering.

### A007 - Bound R-tree node read-ahead

- **Supersedes:** A006 where it requires one-request read-ahead for every known-size node regardless of the maximum encoded span.
- **Replacement:** Single-request node read-ahead is an opportunistic optimization and must be limited to a conservative private maximum span of 1 MiB. If a parsed R-tree `blockSize` would require a larger maximum span, traversal must use the existing exact header-then-declared-body path for that node. The optimization limit must not reject an otherwise parseable file and is not a public configuration option. Verification must cover both sides of the limit.
- **Reason:** A corrupt or unusually large declared block size must not turn latency-oriented read-ahead into an unexpectedly broad allocation or range request.
