import { AllRequests } from "./dataFetchingUtils";
...
export async function executeAllQueries(
  ...
  if (requests.bigRequests.length > 0) {
    promises.push(
      executors.fetchBigData({ variables: { bigRequests: requests.bigRequests } })
    );
  }
  ...
  if (requests.transcriptRequest) {
    promises.push(
      executors.fetchGene({ variables: requests.transcriptRequest })
    );
  }
  ...
  if (requests.motifRequest) {
    promises.push(
      executors.fetchMotif({ variables: requests.motifRequest })
    );
  }
  ...
  if (requests.importanceRequests.length > 0) {
    promises.push(
      executors.fetchImportance({ variables: { bigRequests: requests.importanceRequests } })
    );
  }
  ...
  if (requests.ldRequest) {
    promises.push(
      executors.fetchSnps({ variables: requests.ldRequest })
    );
  }
