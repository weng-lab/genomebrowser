...
import { CustomCollections } from "../custom-tracks/CustomCollections";
...
export function SessionDashboard({
  result,
  customTracks = { status: "storage-unavailable" },
}: {
  ...
  return (
    <Container maxWidth="md" sx={{ py: 4 }}>
      <Stack spacing={3}>
        ...
        <DashboardTabs
          collections={<CustomCollections result={customTracks} />}
          sessions={
            <Stack spacing={3}>
              {result.status === "ready" && (
                <Box>
                  <CreateSessionButton
                    assemblies={assemblies.map(({ definition, label }) => ({
                      id: definition.id,
                      label,
                    }))}
                  ...
                  <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                    {result.sessions.length} of 5 sessions. Delete a session to make room when the
                    limit is reached.
                  </Typography>
                </Box>
              )}
              {result.status === "storage-unavailable" && (
                <Alert severity="info">
                  Session storage is unavailable. You can explore the browser, but your work will
                  not be saved to your account.
                </Alert>
              )}
              {result.status === "error" && (
                <Alert severity="error">
                  Dashboard could not be loaded. Please reload to try again.
                </Alert>
              )}
              {result.status === "ready" &&
