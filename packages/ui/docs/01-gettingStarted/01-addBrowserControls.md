...
[Choose tracks from collections](../02-guides/trackSelection.md) replaces a fixed signal list with a track picker. [Connect a chromosome overview](../02-guides/chromosomeOverview.md) adds a full-chromosome view above the browser.

See the [browser controls reference](../03-reference/01-browserControls/README.md) and [HighlightDialog reference](../03-reference/02-highlights/HighlightDialog.md) for exact props and behavior. If the browser or controls do not respond, start with [troubleshooting](../04-troubleshooting.md).
