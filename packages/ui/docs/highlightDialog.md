...
Select a highlight's pencil action to edit its ID, region, type, color, and opacity inside the dialog. The form starts with the existing values. **Save changes** updates that highlight in place; **Cancel** or closing the dialog discards unsaved edits. IDs must remain unique, but keeping the highlight's current ID is allowed. Invalid input stays in the form for correction.
