...
Start with the symptom below. The [API reference](03-reference/README.md) specifies accepted props and behavior; the [guides](README.md#guides) show complete integrations.
...
Validation also runs while TrackSelect is closed. An application loading external collection data should handle loading and validation errors before rendering the picker. See [collection constraints](03-reference/03-trackSelection/TrackSelect.md#selection-and-validation-constraints).
...
Changing defaults while explicit initial IDs are present changes the Reset target without reinitializing the store. See [initial and reset selections](03-reference/03-trackSelection/TrackSelect.md#set-initial-and-reset-selections).
...
Newly selected tracks follow the active view's grouping and collection source order. Existing tracks retain their relative order. Reset restores the supplied default order in the draft; Submit then applies it. See [TrackSelect](03-reference/03-trackSelection/TrackSelect.md).
...
Saving those IDs preserves selection and order, not changed track settings. Validate restored IDs against the current collections, and scope storage to the relevant assembly and collection set. See [restore and save a selection](02-guides/trackSelection.md#restore-and-save-a-selection).
...
Custom tooltip content must be SVG-compatible. If it appears only on pointer hover, that is expected; keyboard focus does not open tooltips. See [Connect a chromosome overview](02-guides/chromosomeOverview.md) and the [Cytobands reference](03-reference/04-chromosomeOverview/Cytobands.md).
