...
If tooltip content needs additional data, the application owns loading, caching, errors, and cleanup. The tooltip component mounts only while its highlight is active. See the [Cytobands reference](../03-reference/04-chromosomeOverview/Cytobands.md) for tooltip positioning, colors, clipping, and accessibility details.

For missing bands or overlays, see [troubleshooting](../04-troubleshooting.md#the-chromosome-overview-is-empty).
