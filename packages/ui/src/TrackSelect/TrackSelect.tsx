...
export const useSelectionStore = create<SelectionState & SelectionAction>((set) => ({
  ...
  setActive: () => set((state) => (
    // filtering out autogenerated rows and grabbing only first 30 tracks
    { activeTracks: new Set(Array.from(state.selectedIds).filter(i => i[0] === "E").slice(0, 30)) } 
  )), 
  ...
  removeActiveTracks: (removedIds: Set<string>) =>
    set((state) => {
