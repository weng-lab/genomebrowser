...
const ontologyTypes = ["Adipose", "Adrenal gland", "Blood", "Blood vessel", "Bone", "Bone marrow", "Brain", "Breast", "Connective tissue", 
"Embryo", "Epithelium", "Esophagus", "Eye", "Fallopian Tube", "Gallbladder", "Heart", "Kidney", "Large Intestine", "Limb", "Liver", "Lung", 
"Lymphoid Tissue", "Muscle", "Mouth",  "Nerve", "Nose", "Pancreas", "Parathyroid Gland",  "Ovary", "Penis", "Placenta",  "Prostate",  "Skin", 
"Small Intestine", "Spinal Cord", "Spleen", "Stomach", "Testis", "Thymus", "Thyroid", "Urinary Bladder", "Uterus", "Vagina"];
...
function getTracksByAssayAndOntology(assay: string, ontology: string): any[] {
  ...
  data.forEach((track: TrackInfo) => {
    const filteredAssays = track.assays?.filter((e: AssayInfo) => e.assay === assay.toLowerCase()) || [];
    if (filteredAssays.length > 0 && track.ontology === ontology.toLowerCase()) {
      res.push({
        ...track,
...
function formatAssayType(assay: string): string {
  switch (assay) {
    ...
    case "chromhmm": return "ChromHMM";
    default: return assay;
  }
}
...
function flattenIntoRow(track: TrackInfo | FuseResult<any>): RowInfo {
  const { name, ontology, lifeStage, sampleType, displayname } = 'item' in track ? track.item : track;
  const { assay, url, experimentAccession, fileAccession } = 'item' in track ? track.item.assays[0] : track.assays[0];
 ...
 */
function buildTreeView(selectedRows: RowInfo[], root: TreeViewBaseItem<ExtendedTreeItemProps>, sortedAssay: boolean): TreeViewBaseItem<ExtendedTreeItemProps>[] {
  ...
  const topLevelMap = new Map<string, TreeViewBaseItem<ExtendedTreeItemProps>>(); // keep track of top level nodes
  ...
  selectedRows.forEach((row) => {
    ...
    let secondLevelNode = topLevelNode.children!.find(child => child.label === row[secondLevelType]);
    if (!secondLevelNode) {
      secondLevelNode = {
    ...
    if (row.fileAccession) { // only add if fileAccession exists
...
export default function TrackSelect() {
  ...
  const selectedRows = useMemo(() => {
      return rows.filter(row => rowSelectionModel.ids.has(row.experimentAccession));
    }, [rows, rowSelectionModel]);

  // need to memoize treeItems to avoid infinite rerendering of RichTreeView
  const treeItems = useMemo(() => {
    return buildTreeView(selectedRows, { id: "1", label: "Biosamples", icon: "folder", children: [] }, sortedAssay);
  }, [selectedRows]);
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const props: SearchTracksProps = {
      ...
      keyWeightMap: ["displayname", "ontology", "lifeStage", "sampleType", "type", "experimentAccession", "fileAccession"],
      limit: 50
    }
    ...
    <Box width="fit-content">
      <Box display="flex" justifyContent="space-between" sx={{ mb: 3}}>
        <TextField
          ...
          sx={{width: "250px"}}
        />
        <FormControlLabel sx={{ display: 'flex', justifyContent: 'flex-end', mb: 1 }}
          value="Sort by assay"
      ...
      <Stack direction="row" spacing={2}>
        <Paper>
          <Box sx={{ height: '500px', width: '1000px', overflow: 'auto' }}>
            <Typography>
              <Box sx={{ fontWeight: 'bold', padding: 2 }}>{rows.length} available tracks</Box>
            </Typography>
            <DataGridPremium
              rows={filteredRows}
              columns={columns}
          ...
          <Box sx={{ width: '500px', height: '500px', overflow: 'auto' }}>
            ...
            <ThemeProvider theme={theme}>
              <RichTreeView 
                ...
                sx={{ ml: 1, mr: 1, height: 'fit-content', flexGrow: 1, overflowY: 'auto' }}
