...
export type TrackSettingsRangeFieldsProps = RangeLabels &
  (
    | {
        mode: "independent";
        range: YRangeOverride | undefined;
        onCommit: (range: YRangeOverride | undefined) => TrackMutationResult;
      }
    | {
        mode?: "complete";
        range: YRange | undefined;
        onCommit: (range: YRange | undefined) => TrackMutationResult;
      }
  );
...
export function TrackSettingsRangeFields(props: TrackSettingsRangeFieldsProps) {
  ...
  const controller = useDraftController<RangeValues, YRangeOverride | undefined>({
    value: props.range,
    toRaw: toRangeValues,
    ...
    onCommit: (range) => {
      ...
      return props.onCommit(range as YRange | undefined);
    },
  });
  const values = controller.value;
  const hasError = controller.error !== undefined;

  const updateBound = (bound: keyof YRange, value: string) => {
...
function toRangeValues(range: YRangeOverride | undefined): RangeValues {
  return {
...
): DraftValidation<YRangeOverride | undefined> {
...
function rangesAreEqual(left: YRangeOverride | undefined, right: YRangeOverride | undefined) {
