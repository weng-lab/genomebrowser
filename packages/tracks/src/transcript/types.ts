...
export type Exon = { coordinates: { start: number; end: number }; UTRs?: GenomicElement[] };
export type GenomicElement = { coordinates: { start: number; end: number } };
export type RenderedTranscript = {
  transcript: Transcript;
  paths: { introns: string; exons: string };
};
export type TranscriptRow = { y: number; transcripts: RenderedTranscript[] };
