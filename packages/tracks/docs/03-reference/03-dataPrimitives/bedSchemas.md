...
BigBed and BulkBed draw each feature in its parsed color, falling back to the track color when no color is present. Dense rendering may merge adjacent or overlapping features of the same color. Names and scores are available to tooltips and callbacks. Use [cCRE BigBed](../01-trackModules/ccre.md) for a tooltip that also displays cCRE classification.
...
See [BigBed](../01-trackModules/bigbed.md) and [BulkBed](../01-trackModules/bulkbed.md) for track configuration.
