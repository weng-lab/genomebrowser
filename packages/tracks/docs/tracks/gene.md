...
Use `geneModule` for transcript annotations stored in a BigGenePredPlusV1 BigBed file. The track can draw every transcript, only MANE Select transcripts, or one merged structure per gene.
...
| ---------------- | -------- | ----------- | ----------------------------------------------------------------------------- |
| `url`            | `string` | Required    | Non-empty BigGenePredPlusV1 BigBed URL. Changing it requests new data.        |
...
The source must be an absolute public HTTP or HTTPS BigBed URL with the exact BigGenePredPlusV1 columns in this order:
...
`tags` is a comma-separated string. The track trims entries and removes empty and duplicate entries while preserving first-seen order. `attributes` is compact JSON whose top level is an object and whose values are strings or string arrays. The track parses these fields into `GeneTranscript.tags` and `GeneTranscript.attributes`. It exposes trimmed `name2` as `transcriptName`, falling back to the transcript identifier when `name2` is blank. The complete source record, including the serialized fields, remains available as `source`.
...
| `BigGenePredPlusV1Source` | Parsed BigGenePredPlusV1 source fields and original trailing fields.    |
