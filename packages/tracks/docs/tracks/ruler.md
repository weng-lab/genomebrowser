...
Moving the pointer onto the coordinate axis or tick labels temporarily switches Pan mode to Zoom; moving away or finishing/canceling the pointer gesture restores Pan if the ruler made that switch. Gesture completion restores Pan even when zooming replaces the ruler with a loading placeholder. Explicit Zoom and Highlight modes are preserved on entry. The automatic zoom hit area stops above the reference sequence. Sequence characters handle their own pointer hover highlights and retain panning in Pan mode. Pointer movement over dialogs does not activate ruler hover highlights or temporary Zoom mode. Explicit Zoom and Highlight modes still select regions when dragging over sequence characters. Mode changes are reflected in the toolbar. Dragging does not switch modes mid-gesture. Removing the hovered ruler does not restore the previous mode; use the toolbar to choose another mode.
