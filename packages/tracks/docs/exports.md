...
The package has one subpath for each track, plus `/settings` and `/tooltips`. The root exports only `firstPartyTrackModules`.
...
Each module includes its settings component, tooltip component, renderer, and fetcher. The package does not export those track-specific parts separately. Import reusable settings controls and `TrackBaseSettings` from `@weng-lab/genomebrowser-tracks/settings`. Import `TrackTooltip` and its formatters from `@weng-lab/genomebrowser-tracks/tooltips`.
...
Each track subpath also exports that track's data, rendered-item, display, and interaction types. The track pages list them. Interaction aliases use `TrackInteraction<Item, Config>` from core. Pass one as the second argument to `module.create`:
