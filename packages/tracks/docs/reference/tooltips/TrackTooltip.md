...
`TrackTooltipProps` describes the props below. Import it and `TrackTooltipRow` from the same `/shared` entry.
...
See [Tooltip formatters](formatters.md) for formatting signatures and missing-value rules.
