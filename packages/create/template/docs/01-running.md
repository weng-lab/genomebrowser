...
Vite preview inherits this project's search proxy and reads the key from `.env.local`, so you can check search against the built app too. Preview is for testing the build. Permanent hosting requires serving `dist/` and providing the search endpoint separately; see [Deployment](04-deployment.md).
...
- **The page loads but tracks fail.** Genomic URLs are fetched by your laptop's browser. The app tunnel does not forward those data requests. The URLs must be reachable from the laptop, including any required VPN, and support CORS and byte-range requests. See [Host your genomic files](04-deployment.md#host-your-genomic-files).
