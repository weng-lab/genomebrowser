...
Replace `YOUR_URL_HERE` with your data URL. The instance has resolved display, height, color, source, and configuration defaults. Pass it to [createTrackStore](../browserSetup/trackStore.md) or a store action. The inferred helper types are specified with [TrackModule](defineTrackModule.md#trackmodule-and-inferred-types).
