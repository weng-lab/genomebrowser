...
Validation checks object shape, coordinates, ordering, membership, then overlap. Expected invalid-region inputs return a failure result rather than throwing. To commit a region to a browser, see [browser-store navigation](../01-browserSetup/browserStore.md#navigation).
