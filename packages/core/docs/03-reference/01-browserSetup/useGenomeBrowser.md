...
`getState()` reads a snapshot without subscribing. Use it when an event needs a current value that does not drive rendering. See [state and action access](../../01-gettingStarted/01-firstBrowser.md#access-browser-state-and-actions) for examples.
