...
Boundary clamping can shorten the requested region. A successful store mutation updates state synchronously; it does not wait for mounted tracks to finish fetching or rendering. See [request behavior](../03-trackDefinition/fetchingData.md#requests-and-result-lifetime).
...
See [selection interactions](../../01-gettingStarted/03-navigationAndSelection.md#select-a-region-by-dragging) for pointer behavior, cancellation, and keyboard responsibilities.
