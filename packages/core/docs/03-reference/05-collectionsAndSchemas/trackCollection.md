...
[Validation](validateTrackCollection.md) does not add tracks to a browser. Create all entries or only those your application selects, then use a [track-store action](../01-browserSetup/trackStore.md).
...
| `base`     | `TrackBaseInput`          | Required | Required ID and title, plus optional display, height, and color. See [creation input](../03-trackDefinition/trackInstances.md#trackcreateinput-and-trackbaseinput). |
