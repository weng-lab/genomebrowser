...
Replace the URL with a browser-accessible cCRE BigBed file for the browser's assembly, and pass `useTrackStore` to `GenomeBrowser`. To offer the variant through a [collection](../gettingStarted/trackCollections.md), register it in the collection's module set and use `type: "application-ccre"` in its entries. The tooltip and settings remain module behavior in application code; the collection stores their configuration values.

## Further reading

- [Module definitions](../reference/trackDefinition/defineTrackModule.md): definition options, defaults, and creation contracts.
- [Track settings](../reference/trackDefinition/trackSettings.md): settings components and validated updates.
- [Tooltips](../reference/rendererIntegration/useTooltip.md): tooltip components and runtime context.
