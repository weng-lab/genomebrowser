...
This chapter uses the browser from [Add and configure tracks](02-configureTracks.md). The examples assume its shared stores in `browserState.ts`; [state and action access](01-firstBrowser.md#access-browser-state-and-actions) covers how to use those stores from React.
...
For known regions, `addHighlight` adds a highlight with an application-chosen ID, region, and style. `removeHighlight` removes it by ID. Include the chromosome in the region to keep the highlight specific to that chromosome. An existing ID makes `addHighlight` a no-op rather than replacing that entry. The [highlight reference](../03-reference/01-browserSetup/browserStore.md#highlights) describes the stored shape and validation behavior.
...
[Use track collections](04-trackCollections.md) describes reusable track catalogs and how to load them directly or through TrackSelect. The [browser-store reference](../03-reference/01-browserSetup/browserStore.md) contains the complete navigation, selection, and highlight contracts.
