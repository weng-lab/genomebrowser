...
For custom modules, wrap every config schema field that changes the request or fetch-time processing with `fetchOnChange`. Region, SVG width, assembly, and display changes request data, but a config-only change requests a track only when its marked-field signature changes.
