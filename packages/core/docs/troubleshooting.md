...
The [first browser example](gettingStarted/firstBrowser.md) shows the responsive wrapper. The [sizing reference](reference/browserSetup/GenomeBrowser.md) covers fixed dimensions and magnification.
...
Settings-dialog callbacks can also return `INTERACTION_BLOCKED` while browser interactions are disabled. Treat that as a rejected edit rather than assuming it was queued. The [track-store reference](reference/browserSetup/trackStore.md) lists mutation results and patch behavior.
...
A remounted visualization also recreates its internal request and overlay state, even if its supplied stores survive. This can explain a new fetch without a reset to the application region. [State and browser lifetime](guides/stateAndLifetime.md) shows both ownership patterns.
...
For reusable track components, keep the hook inside the hosted component rather than at module-definition time or inside a fetcher. The [context reference](reference/browserSetup/useGenomeBrowser.md) explains how resolving a store differs from subscribing to it.
...
`onCommittedTrackIds` saves successful Submit selections only. It does not observe initialization, Cancel, or changes from other controls, and it excludes non-collection tracks. [Use track collections](gettingStarted/trackCollections.md) explains which selection changes the callback saves.
