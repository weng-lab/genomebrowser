...
describe("track data controller", () => {
  ...
  it("refetches a zoom when chromosome clipping keeps the fetched region unchanged", async () => {
    const { controller, browserStore } = setup({
      trackIds: ["a"],
      region: { chromosome: "chr1", start: 0, end: 600 },
      chromosomeLength: 1_000,
    });
    const first = lastRequest("a").context.demand;
    await resolveAll();

    browserStore.getState().setRegion({ chromosome: "chr1", start: 0, end: 500 });

    expect(fetch).toHaveBeenCalledTimes(2);
    expect(lastRequest("a").context.demand.region).toEqual(first.region);
    expect(lastRequest("a").context.demand.visibleRegion).toEqual({
      chromosome: "chr1",
      start: 0,
      end: 500,
    });
    expect(controller.getTrack("a")).toEqual({ status: "loading" });
    await resolve("a", "zoomed");
    expect(data(controller, "a")).toBe("zoomed");
  });
