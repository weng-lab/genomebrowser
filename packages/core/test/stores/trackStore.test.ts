...
it("merges nested config patches, replaces arrays, and validates the entire result atomically", () => {
  ...
  const track = module.create({
    base: { id: "nested", title: "Nested" },
    config: {
      display: { height: 14, colors: ["red", "blue"], nested: { a: 1, b: 2 } },
      optional: "value",
    },
  });
  const store = createTrackStore({ modules: [module], tracks: [track] });
  expect(
    store.getState().updateTrack("nested", {
      config: { display: { colors: ["green"], nested: { a: 3 } }, optional: undefined },
    }).ok,
  ).toBe(true);
  expect(store.getState().getTrack("nested")?.config).toEqual({
    display: { height: 14, colors: ["green"], nested: { a: 3, b: 2 } },
    optional: undefined,
  });
  expect(track.config.display.nested).toEqual({ a: 1, b: 2 });
  const before = store.getState();
  expect(
    before.updateTrack("nested", { config: { display: { height: -1, nested: { b: 5 } } } }).ok,
  ).toBe(false);
  expect(store.getState()).toBe(before);
});
