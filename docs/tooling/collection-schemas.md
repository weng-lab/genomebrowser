...
If the tracks build fails because the old schema has the wrong track count, build core and reader through Turbo, then run `pnpm --dir packages/tracks exec vite build` to refresh the module bundle before regenerating. Finish with formatting and `pnpm verify`.
